...
- **`claude-extract` — one-command Claude backend** (ADR-0005 / #20, reverses C3) — a thin script over headless `claude -p` that assembles the canonical prompt + image, best-effort-parses the `{raw_text, fields}` reply, and hands it to `claude_envelope.write_from_payload` (reused, not reimplemented). Takes a deliberate dependency on the `claude` CLI (+ per-run cost) for reproducibility; the F5 convention stays as the no-dependency fallback. Scrubs its own venv from the spawned `claude`'s env so the nested session's hooks aren't perturbed. Records `--model` verbatim; real `duration_sec` from the CLI's `duration_ms`.
